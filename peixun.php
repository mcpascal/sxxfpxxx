...
<div class="main">
	<div class="main-1">
		...
		?>
		<Div class="main-2">
			<div class="left"><img src="images/left-top-2.gif" />
				<div class="left-k">
					<ul>
					<?php
					$singlePageData=new SinglePageData();
					$arr=$singlePageData->listAllBySinglePageClassId($classId);
					for($i=0;$i<count($arr);$i++){
					?>
					<li<?php if($arr[$i]->id==$id){echo(" class=\"cu\"");}?>><a href="?id=<?php echo($arr[$i]->id)?>"><?php echo($arr[$i]->title)?></a></li>
					<?php
					}
					?>
					</ul>
			...
			<div class="right">
				<div class="right-top">
					...
					<div class="home"><img src="images/home.gif" />&nbsp;&nbsp;您当前位置：网站首页 > 培训项目 > <?php echo($title)?></div>
				</div>
