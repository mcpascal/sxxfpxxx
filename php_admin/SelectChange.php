...
if($act=="ok"){
	...
	$select=new Select($_POST["classname"],$_POST["type"],$_POST["title"],$_POST["a"],$_POST["b"],$_POST["c"],$_POST["d"],$answer,$id);
	$selectData->change($select)?done(1,"SelectManage.php"):done(0);
}
?>
<body>
<div class="title">
	<div class="title1">
		...
		<div class="title1Center">选择题修改</div>
	</div>
</div>
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	...
	<tr>
		...
		foreach($classArr as $c){
			echo '<option value="' . $c . '"';
			if($select->classname == $c){echo 'selected="selected"';}
			echo '>' . $c . '</option>';
		}
		?>
		</select>
	...
	<tr>
		...
		<td align="left">
		<input type="radio" name="type" value="1" checked="checked" />单选&nbsp;&nbsp;&nbsp;<input name="type" type="radio" value="2"<?php if($select->type == 2){echo(' checked="checked"');}?> />多选
		</td>
	</tr>
	<tr>
		<td align="right" class="tdLeft">题目：<span class="red">*</span></td>
		<td align="left"><input type="text" class="text500" value="<?php echo($select->title);?>" name="title"></td>
	</tr>
	<tr>
		<td align="right" class="tdLeft">选项A：<span class="red">*</span></td>
		<td align="left">
			<input type="text" class="text500" value="<?php echo($select->a);?>" name="a">
		</td>
	</tr>
	<tr>
		<td align="right" class="tdLeft">选项B：<span class="red">*</span></td>
		<td align="left">
			<input type="text" class="text500" value="<?php echo($select->b);?>" name="b">
		</td>
	</tr>
	<tr>
		<td align="right" class="tdLeft">选项C：<span class="red">*</span></td>
		<td align="left">
			<input type="text" class="text500" value="<?php echo($select->c);?>" name="c">
		</td>
	</tr>
	<tr>
		<td align="right" class="tdLeft">选项D：<span class="red">*</span></td>
		<td align="left">
			<input type="text" class="text500" value="<?php echo($select->d);?>" name="d">
		</td>
	</tr>
	<tr>
		<td align="right" class="tdLeft">答案：<span class="red">*</span></td>
		<td align="left">
			<input type="checkbox" value="a" name="answer[]"<?php if(strpos($select->answer, "a") !== false){echo(' checked="checked"');}?> />A
			<input type="checkbox" value="b" name="answer[]"<?php if(strpos($select->answer, "b") !== false){echo(' checked="checked"');}?> />B
			<input type="checkbox" value="c" name="answer[]"<?php if(strpos($select->answer, "c") !== false){echo(' checked="checked"');}?> />C
			<input type="checkbox" value="d" name="answer[]"<?php if(strpos($select->answer, "d") !== false){echo(' checked="checked"');}?> />D
		</td>
	</tr>
	<tr>
		<td></td>
