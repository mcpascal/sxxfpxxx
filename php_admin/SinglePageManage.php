...
var del=new Delete("aSelectAll","SinglePageDel.php");
</script>
</head>
...
<div class="title">
	<div class="title1">
		...
		<div class="title1Center">单页面管理</div>
	</div>
</div>
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	...
	$sql="select * from tbl_singlePage order by id desc";
	$singlePageData=new SinglePageData();
	$arr=$singlePageData->listBySql($sql,$page);
	...
	?>
		<tr id="tr<?php echo($i)?>" onMouseOver="javascript:changeTrbk('<?php echo($i);?>');" onMouseOut="javascript:changeTrbk1('<?php echo($i);?>');">
			...
			<td align="center">
			<?php 
			$singelPageClassData=new SinglePageClassData();
			$arr1=$singelPageClassData->getInfoById($arr[1][$i]->singlePageClassId);
			$arr1[0]?print($arr1[1]->name):print("未找到该类别");
			?>
			</td>
			<td align="center"><?php echo($arr[1][$i]->title);?></td>
			<td align="center"><?php echo($arr[1][$i]->sort);?></td>
			<td align="center"><a href="SinglePageChange.php?id=<?php echo($arr[1][$i]->id);?>">修改</a>&nbsp;&nbsp;<a href="javascript:del.deleteOne('<?php echo($arr[1][$i]->id);?>','tr<?php echo($i)?>')">删除</a></td>
		</tr>
	<?php
	}
	?>
