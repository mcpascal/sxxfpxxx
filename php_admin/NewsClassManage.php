...
if($act=="add"){
	...
	$newsClass=new NewsClass($_POST["name"],0,0,"",1,$_POST["sort1"]);
	$newsClassData=new NewsClassData();
	if(is_numeric($_POST["upClassId"])){
		$newsClassData->addByUpClassId($_POST["upClassId"],$newsClass)?done(1,"NewsClassManage.php"):done(0);
	}else{
		$newsClassData->add($newsClass)?done(1,"NewsClassManage.php"):done(0);
	}
}
if($act=="change"){
	$checkValueNotAlert=new CheckValueNotAlert();
	...
	$newsClass=new NewsClass($_POST["name".$id],0,0,"",0,$_POST["sort".$id],(int)$id);
	$newsClassData=new NewsClassData();
	$newsClassData->change($newsClass)?done(1,"NewsClassManage.php"):done(0);
}
...
if($act=="del"){
	...
	$checkValueNotAlert->checkReg("编号",$id,"/^\d{1,}$/");
	$newsClassData=new NewsClassData();
	if($newsClassData->delete($id)){
		$newsData=new NewsData();
		$newsData->deleteByNewsClassId($newsClassData->idStr)?done(1,"NewsClassManage.php"):done(0);
	}else{
		done(0);
	}
}
...
<div class="title">
	<div class="title1">
		...
		<div class="title1Center">新闻类别添加</div>
	</div>
</div>
...
<div class="title" style="margin-top:10px;">
	<div class="title1">
		<div class="title1Left"><img src="images/smallimg/2.png" /></div>
		<div class="title1Center">新闻类别管理</div>
	</div>
</div>
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	<tr class="tdTitle">
	...
	?>
		...
		<tr id="tr<?php echo($i+1)?>" onMouseOver="javascript:changeTrbk('<?php echo($i+1);?>');" onMouseOut="javascript:changeTrbk1('<?php echo($i+1);?>');">
			...
			<td align="center">
				...
				<span style="cursor:pointer" onclick="del('NewsClassManage.php?act=del&id=<?php echo($arr[$i]->id);?>');">删除</span>
			</td>
		</tr>
		</form>
	<?php
