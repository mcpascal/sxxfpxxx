...
var del=new Delete("aSelectAll","SelectDel.php");
</script>
</head>
...
<div class="title">
	<div class="title1">
		...
		<div class="title1Center">选择题管理</div>
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	...
	?>
		<tr id="tr<?php echo($i)?>" onMouseOver="javascript:changeTrbk('<?php echo($i);?>');" onMouseOut="javascript:changeTrbk1('<?php echo($i);?>');">
			<td align="center" width="50"><input type="checkbox" name="checkbox" id="checkbox" value="<?php echo($arr[1][$i]->id);?>_<?php echo($i)?>" /></td>
			<td align="center" width="50"><?php echo($arr[1][$i]->id);?></td>
			<td align="center" width="200"><?php echo($arr[1][$i]->classname);?></td>
			...
			<td align="center" width="100"><a href="SelectChange.php?id=<?php echo($arr[1][$i]->id);?>">修改</a>&nbsp;&nbsp;<a href="javascript:del.deleteOne('<?php echo($arr[1][$i]->id);?>','tr<?php echo($i)?>')">删除</a></td>
		</tr>
	<?php
	}
	?>
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	<tr>
		...
		?>
			<option value="?page=<?php echo($k)?>&key=<?php echo($key);?>" <?php if ($k==$arr[0]->currentPage){ ?> selected="selected"<?php }?>><?php echo($k);?></option>
		<?php
			}
		?>
		</select>页  
